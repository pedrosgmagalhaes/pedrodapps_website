...
self.onmessage = async (e) => {
  ...
  if (type === 'install') {
    try {
      ...
      self.postMessage({ type: 'install_result', ok: true, message: `Installed: ${pkgs.join(', ')}` });
    } catch (err) {
      self.postMessage({ type: 'install_result', ok: false, error: String(err && err.message ? err.message : err) });
    }
    return;
  }
};